...
def _timedeltas_to_days(timedeltas):
    ...
    if np.any(dt_series <= 0):
        raise Py4DGeoError(
            "Kalman filtering requires positive temporal step lengths."
        )
...
def _kalman_smooth_series(
    ...
    if observation_sigma.shape != observations.shape:
        raise ValueError(
            "observation_sigma must have the same shape as observations."
        )

    if dt_series.shape != observations.shape:
        raise ValueError(
            "dt_series must have the same length as observations."
        )
    ...
    kf.x = np.asarray(
        [observations[first_valid], 0.0],
        dtype=float,
    )
    kf.P = np.eye(2, dtype=float)
    kf.H = np.asarray([[1.0, 0.0]], dtype=float)

    for epoch_index in range(n_epochs):
        dt = float(dt_series[epoch_index])
        ...
        if np.isfinite(observations[epoch_index]):
            sigma = max(
                float(observation_sigma[epoch_index]),
                np.finfo(float).eps,
            )
            kf.R = np.asarray([[sigma**2]], dtype=float)
            kf.update(
                np.asarray(
                    [observations[epoch_index]],
                    dtype=float,
                )
            )

        filtered_states[epoch_index] = kf.x
        filtered_covariances[epoch_index] = kf.P

    smoothed_states = filtered_states.copy()
    smoothed_covariances = filtered_covariances.copy()

    for epoch_index in range(n_epochs - 2, -1, -1):
        dt = float(dt_series[epoch_index + 1])
        ...
        predicted_covariance = (
            transition
            @ filtered_covariances[epoch_index]
            @ transition.T
            + process_covariance
        )
        ...
        smoothed_states[epoch_index] = (
            filtered_states[epoch_index]
            + smoother_gain
            @ (
                smoothed_states[epoch_index + 1]
                - transition @ filtered_states[epoch_index]
            )
        )

        smoothed_covariances[epoch_index] = (
            filtered_covariances[epoch_index]
            + smoother_gain
            @ (
                smoothed_covariances[epoch_index + 1]
                - predicted_covariance
            )
            @ smoother_gain.T
        )

    change = smoothed_states[:, 0]
    rate = smoothed_states[:, 1]

    sigma_change = np.sqrt(
        np.clip(
            smoothed_covariances[:, 0, 0],
            0,
            None,
        )
    )
...
def _find_formation_start(
    ...
    earliest_allowed_epoch = int(earliest_allowed_epoch)
    ...
    search_start = max(
        earliest_allowed_epoch,
        raw_start_epoch - int(max_extension_epochs),
    )
    ...
    signal = change[candidate_epochs]
    sigma = change_sigma[candidate_epochs]

    baseline_threshold = np.maximum(
        float(baseline_tolerance),
        float(baseline_sigma_factor) * sigma,
    )

    if direction == "positive":
        same_sign = signal >= -float(baseline_tolerance)
    elif direction == "negative":
        same_sign = signal <= float(baseline_tolerance)
    else:
        return raw_start_epoch

    near_baseline = np.abs(signal) <= baseline_threshold

    candidates = candidate_epochs[
        same_sign & near_baseline
    ]

    if candidates.size:
        extended_start = int(candidates[-1])
    else:
        ...
        finite_epochs = candidate_epochs[finite]
        finite_signal = signal[finite]

        extended_start = int(
            finite_epochs[
                np.argmin(np.abs(finite_signal))
            ]
        )
    ...
    if extended_start < raw_start_epoch - 1:
        ...
        if direction == "positive":
            consistent_fraction = np.mean(
                slopes >= -float(min_extension_slope)
            )
        else:
            consistent_fraction = np.mean(
                slopes <= float(min_extension_slope)
...
class KalmanRegionGrowingAlgorithm(
    ...
    """
    Kalman-enabled 4D-OBC algorithm integrated directly into
    py4dgeo's RegionGrowingAlgorithm architecture.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if detection_mode not in (
            ...
            raise ValueError(
                "detection_mode must be 'magnitude' or 'rate'."
            )
        ...
        if seed_direction not in (
            ...
            raise ValueError(
                "seed_direction must be 'positive', 'negative', or 'both'."
            )
        ...
        self.min_seed_duration = int(min_seed_duration)
        self.min_seed_magnitude = float(min_seed_magnitude)
        self.seed_direction = seed_direction
        self.kalman_corepoint_indices = kalman_corepoint_indices
        ...
        self.use_spatial_support = bool(use_spatial_support)
    ...
    def _selected_corepoints(self):
        n_corepoints = (
            self.analysis.distances.shape[0]
        )

        if self.kalman_corepoint_indices is None:
            return np.arange(
                n_corepoints,
                dtype=int,
            )
        ...
        indices = np.asarray(
            list(self.kalman_corepoint_indices),
            dtype=int,
        )

        if indices.ndim != 1:
            raise Py4DGeoError(
                "kalman_corepoint_indices must be one-dimensional."
            )

        if (
            np.any(indices < 0)
            or
            np.any(indices >= n_corepoints)
        ):
            raise Py4DGeoError(
                "kalman_corepoint_indices contains an invalid corepoint index."
            )
    ...
    def _run_kalman_filter(self):
        ...
        if distances.ndim != 2:
            raise Py4DGeoError(
                "analysis.distances must have shape (n_corepoints, n_epochs)."
            )
        ...
        if uncertainties is None:
            raise Py4DGeoError(
                "SpatiotemporalAnalysis contains no M3C2 uncertainties."
            )

        if (
            uncertainties.dtype.names is None
            or
            "lodetection"
            not in uncertainties.dtype.names
        ):
            raise Py4DGeoError(
                "analysis.uncertainties does not contain 'lodetection'."
            )
        ...
        try:
            ...
            raise Py4DGeoError(
                "M3C2 LoDetection values cannot be broadcast to analysis.distances."
            ) from exc
        ...
        if (
            ...
            raise Py4DGeoError(
                "Number of analysis timedeltas does not match the number of distance epochs."
            )
        ...
        self.kalman_change = change
        self.kalman_rate = rate
        self.kalman_sigma_change = sigma_change
        self.kalman_sigma_rate = sigma_rate
        self.kalman_observation_sigma = (
            observation_sigma
        )
        self.kalman_dt_days = dt_series


    def _detect_seed_records(self):
        timestamps = [
            self.analysis.reference_epoch.timestamp + td
            for td in self.analysis.timedeltas
        ]
        ...
        for cp_idx in selected_indices:
            ...
            change = self.kalman_change[cp_idx]
            rate = self.kalman_rate[cp_idx]
            sigma_change = (
                self.kalman_sigma_change[cp_idx]
            )
            sigma_rate = (
                self.kalman_sigma_rate[cp_idx]
            )

            if np.all(np.isnan(change)):
                continue

            if self.detection_mode == "magnitude":
                signal = change
                signal_sigma = sigma_change
            else:
                signal = rate
                signal_sigma = sigma_rate
                    ...
                    if (
                        ...
                        start_epoch = (
                            _find_formation_start(
                                change=change,
                                change_sigma=sigma_change,
                                raw_start_epoch=raw_start,
                                direction=direction,
                                earliest_allowed_epoch=earliest_allowed,
                                baseline_tolerance=self.formation_baseline_tolerance,
                                baseline_sigma_factor=self.formation_baseline_sigma_factor,
                                max_extension_epochs=self.max_formation_extension_epochs,
                                min_extension_slope=self.min_extension_slope,
                            )
                        )
    ...
    def _apply_spatial_support(
        ...
        support_time_tolerance expands only the validation window.
        It does NOT change the actual seed interval used for
        RegionGrowingSeed.
        """
        if (
            seed_table is None
        ...
        corepoints = np.asarray(
            self.analysis.corepoints.cloud,
            dtype=float,
        )
        ...
        for _, seed in (
            ...
            for neighbour in neighbours:
                ...
                if direction == "positive":
                    ...
                    supports_event = (
                        ...
                        neighbour_value
                        >= (
                            self.neighbour_magnitude_ratio
                            * event_magnitude
                        )
                    )
            ...
            # Keep ORIGINAL Kalman seed interval
            record[
                "support_start_epoch"
            ] = start_epoch
            ...
            # Store expanded validation window separately
            record[
                "validation_start_epoch"
            ] = validation_start_epoch
    ...
    def _apply_nms(
        ...
        for row_index, seed in (
            ...
            if (
                support_count
                < self.nms_min_support_count
            ):
                continue

            if (
                support_fraction
                < self.nms_min_support_fraction
            ):
                continue
    ...
    def find_seedpoints(self):
        ...
        self.seed_table = seed_table.copy()
